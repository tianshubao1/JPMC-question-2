...
if __name__ == '__main__':
    print("Visualizing OT Transport Matrix...")
    ...
    # --- 2. SETUP DATA ---
    # Source: 1D particles arranged linearly from -5 to 5
    N = 20
    particles = tf.linspace(-5.0, 5.0, N)[:, None]
    particles = tf.cast(particles, tf.float32)
    particles = particles[None, :, :]  # Batch=1

    # Weights: Concentrated on edges (U-shaped distribution)
    # This creates a mismatch with the Uniform target
    w_logits = tf.abs(particles[0, :, 0])
    log_w = tf.math.log(tf.nn.softmax(w_logits)[None, :])

    # --- 3. COMPUTE OT PLAN ---
    # epsilon=0.1 provides enough regularization to see the "fuzziness" (gradients)
    ...
    # Plot A: Weights
    ax[0].bar(range(N), weights_np, alpha=0.6, color='blue', label='Source Weights')
    ax[0].axhline(y=1 / N, color='r', linestyle='--', linewidth=2, label='Target (Uniform)')
    ax[0].set_title("Particle Weight Distribution")
    ...
    ax[1].set_title("Transport Plan $P$ (Source $\\to$ Target)")
    ax[1].set_xlabel("New Particle Index (Target)")
    ax[1].set_ylabel("Old Particle Index (Source)")
    plt.colorbar(im, ax=ax[1], label='Transport Mass')

    plt.tight_layout()

    plt.savefig("fig_ot_transport.png")
